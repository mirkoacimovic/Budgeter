...
<section class="max-w-2xl mx-auto mt-12 p-4 bg-white shadow-md border border-gray-200 rounded">
    <form action="/register" method="POST" class="grid grid-cols-1 gap-6">
        ...
        <label class="block">
            ...
            <input value="<?php echo e($oldFormData['email'] ?? ''); ?>" name="email" type="email" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" />
            <?php if (array_key_exists('email', $errors)) : '' ?>
                <div class="bg-gray-100 mt-2 p-2 text-red-500">
                    <?php
                    for ($i = 0; $i < count($errors['email']); $i++) {
                        echo $errors['email'][$i] . '<br/>';
                    }
                    ?>
                </div>
            <?php endif; ?>
        ...
        <label class="block">
            ...
            <select name="country" class="block w-full mt-1 rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                ...
                <option value="Canada" <?php if (array_key_exists('country', $oldFormData)) echo $oldFormData['country'] === 'Canada' ? 'selected' : ''; ?>>Canada</option>
                <option value="Mexico" <?php if (array_key_exists('country', $oldFormData)) echo $oldFormData['country'] === 'Mexico' ? 'selected' : ''; ?>>Mexico</option>
